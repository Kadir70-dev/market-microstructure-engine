...
A C++ market-intelligence engine. Every 30s it reads live quotes from a CSV an
MQL5 EA exports out of MT5-under-Wine (the sole feed), runs them through a
...
|---|---:|---|
| Architecture & data model | 8 | Immutable ground truth, clean module split, single provider seam |
...
```bash
# deps (Ubuntu) — only spdlog + sqlite3 (MT5-only via file-export; no HTTP/JSON)
...
# point the engine at the MQL5 EA's CSV (absolute Wine-prefix path in prod)
MME_QUOTES_CSV=/abs/.../MQL5/Files/mme_quotes.csv ./engine
MME_DB_PATH=../data/verify.db ./engine        # separate DB for a test run
...
│   ├── market_data/        provider.hpp (interface), file_provider (reads the EA's CSV — only provider)
...
│
├── src/
│   ├── main.cpp            Control loop, signal handling, log setup, FileProvider wiring, processSymbol().
│   ├── market_data/        file_provider.cpp — reads mme_quotes.csv (mtime = freshness), returns mid.
...
│   ├── mt5_bridge/         mt5_file_export.mq5 — MQL5 EA that writes mme_quotes.csv (the SOLE feed).
│   │                       Runs inside MT5-under-Wine; read-only, no order calls. See docs/MT5_BRIDGE.md.
...
│
├── data/mme_quotes.csv     MQL5 EA output (the feed) — gitignored. symbol,epoch,bid,ask,mid.
...
Architecture: **MT5 (Wine) → MQL5 EA → mme_quotes.csv → FileProvider → SQLite.**

`agent/mt5_bridge/mt5_file_export.mq5` runs INSIDE the MT5 terminal (which runs
fine under Wine) and overwrites `MQL5/Files/mme_quotes.csv` every few seconds
(schema `symbol,epoch,bid,ask,mid`). The native-Linux engine's `FileProvider`
(`src/market_data/file_provider.cpp`) reads it; staleness = file mtime older
than `MME_FILE_STALE_S` (default 120s) → quotes skipped.

Read-only by construction: the EA has no order calls (warns on a non-demo
account) and the engine only reads a file. Check the feed:
`ops/check_quotes.sh [path]` (exit 0 fresh / 1 missing / 2 stale). Full setup:
`docs/MT5_BRIDGE.md`.
...
| `check_quotes.sh` | One-shot freshness check of the EA's CSV (0 fresh / 1 missing / 2 stale). |
...
3. **Read-only stays.** Don't add order calls to the EA, and never make the
   engine write anywhere but its own SQLite. The EA's non-demo warning stays.
4. **Backtest gates stay.** Don't relax the look-ahead/staleness exclusions to
   make numbers look better.
5. **Never commit secrets.** `ops/.env` is gitignored. No API key exists anymore
   (MT5-only). The CSV feed (`data/mme_quotes.csv`) is gitignored too.
...
- **Engine won't start:** `cat logs/engine.err`; confirm you're in `build/`. (A
  missing/stale CSV does NOT stop the engine — it warns and retries each cycle.)
...
- **Feed looks frozen:** `health_check.py` `price_not_frozen` WARN. Could be
  ...
  `ops/check_quotes.sh` and the MT5 terminal/EA (is it running under Wine?).
...
- **No quotes / feed stale:** `ops/check_quotes.sh` — is MT5 + the EA running
  under Wine, and is `MME_QUOTES_CSV` pointed at the right Wine-prefix path?
...
4. **MT5-under-Wine is a heavier feed** than a hosted REST API: the terminal +
   EA must stay running. A stall is detected (file mtime), not silent.
